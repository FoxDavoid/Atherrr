...
//.setDescription(`${client.emoji.cross} | You didn't mentioned the member whom you want to mute.`)
    ...
    if(minutes < 1){
      return message.reply({embeds: [
        new EmbedBuilder()
        .setColor(client.color)
  .setDescription(`${client.emoji.cross} | Noup, that's less than \`1m\`.`)]})
    }
...
async function timeout(issuer, target, minutes, reason) {
  ...
  else if (response === "ALREADY_TIMEOUT") return getEmbed(`${target.client.emoji.cross} | <@${target.user.id}> is already muted! (ha)`, target.client);
