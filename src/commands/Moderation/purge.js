...
//.setDescription(`${client.emoji.cross} | You must provide the number of messages to be deleted.`)
              .setAuthor({ name: `${client.user.username}`, iconURL: client.user.displayAvatarURL({dynamic: true})})
                          .setTitle("**__Ather's OsPurge Messages Cmd__**")
      .setColor('#2f3136')
       ...
       if (!parseInt(amount)) {
          message.reply({embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | You must provide a number of messages.`)]})
       } else if (amount >= 1000) {
         message.reply({embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | You can't delete more than **999** messages at a time.`)]})
          ...
          message.channel.send({embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.tick} | Successfully deleted ${amount} messages (no one will find the proof).`)]})
        }
      }
    }
  }
