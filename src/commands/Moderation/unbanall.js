...
module.exports = {
    ...
    description: "to unban all user from a server (wtf)",
    args: false,
    usage: "",
    userPerms: [],
    owner: false,
        ...
        message.guild.bans.fetch().then((bans) => {
            if (bans.size == 0) {
                const embed = new EmbedBuilder()
                  .setAuthor({name: `Ather's Unbanall Panel`, iconURL:  client.user.displayAvatarURL()})
 ...
 ! There are no banned users.`)
    ...
    const sai = new EmbedBuilder()
                  .setDescription(`\<:x_owner:1239384616624722010>
  ** Unbaning all ${banned} members ** (lmao)`)
                  .setAuthor({name: `Ather's Unbanall Panel`, iconURL: client.user.displayAvatarURL()})
                  .setColor("#2f3136");
