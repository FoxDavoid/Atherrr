...
module.exports = {
    ...
    execute: async (message, args, client, prefix) => {
      ...
      if(user.id === client.user.id) return message.reply({embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | You can't kick me, son of a bitch.`)]})
      
      if(user.id === message.guild.ownerId) return message.reply({embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | I can't kick the owner of this server, lmaaaooo.`)]})
