...
if(!list){
    const embed = new EmbedBuilder()
    .setAuthor({name: `Ather's RoleSetup Panel`, iconURL: client.user.avatarURL({dynamic: true})})
.setTitle("**__Ather's RoleSetup Cmd__**")
...
if(girl === 'na'){
    ...
    const embed = new EmbedBuilder()
        .setColor(client.color)
      .setFooter({text: `Requested by` + message.author.tag, iconURL: message.author.displayAvatarURL()})
    .setAuthor({name: `Ather's RoleSetup Panel`,iconURL: client.user.displayAvatarURL()})
